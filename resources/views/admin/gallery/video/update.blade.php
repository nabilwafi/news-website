      ...
      <div class="card-body">
        ...
        <form class="forms-sample" action="{{ route('update.video', $video->id) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="exampleInputUsername1" name="title" value="{{ $video->title }}">
            @error('title')
          ...
          <div class="form-group">
            ...
            <textarea name="embed_code" id="summernote" cols="30" rows="10">{{ $video->embed_code }}</textarea>
            @error('embed_code')
              <span class="text-danger">{{ $message }}</span>
            @enderror
          </div>
